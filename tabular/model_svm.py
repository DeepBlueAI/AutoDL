...
To create a valid submission, zip model_dnn.py together with other necessary files
...
class Model(object):
    ...
    """
        ...
        # Classifier using sklearn svm
        # self.model_svm = CalibratedClassifierCV(LinearSVC(random_state=0, tol=1e-5, max_iter=100))
        self.model_svm = LinearSVC(random_state=0, tol=1e-5, max_iter=100)
    ...
    ****************************************************************************
    ...
    """
        ...
        if not hasattr(self, 'num_examples_train'):
            ...
            # examples, labelss = iterator.get_next()
            ...
            with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
                while True:
                    try:
                        ...
                        # example, labels = sess.run(examples), sess.run(labelss)
                        # X.append(example)
                        # Y.append(labels)
                        ...
                        break
            print(f'len(X): {len(X)} , collen(X): {len(X[0])}')
            self.X_train = np.array(X)
            self.y_train = np.argmax(Y, axis=1)
            # np.savetxt('train.csv', self.X_train, delimiter=',')
            # np.savetxt('label.csv', self.y_train, delimiter=',')
            ...
            logger.info("Finished counting. There are {} examples for training set." \
                        .format(sample_count))
        print(f'spand time: {time.time()-t1}')
    ...
    def class2onehot(self, labels_dense):
