...
from model_manger import SklearnLGB
...
class Model(object):
    ...
    """
        ...
        self.model_lgb = SklearnLGB(self.output_dim)
    ...
    ****************************************************************************
    ...
    """
        ...
        if not hasattr(self, 'num_examples_train') and self.step != 0:
        ...
        if self.step == 1:
            self.model_lgb.valid_fit(self.X_train, self.y_train)
    ...
    """
        ...
        if not hasattr(self, 'num_examples_test') and self.step != 0:
        ...
        if self.step == 1:
            test_results = self.model_lgb.predict(self.X_test)
    ...
    def class2onehot(self, labels_dense):
        ...
        # num_classes = np.max(labels_dense)+1
        index_offset = np.arange(num_labels) * self.output_dim
        labels_one_hot = np.zeros((num_labels, self.output_dim))
