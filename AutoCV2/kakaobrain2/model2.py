...
import skeleton2
from architectures2.resnet import ResNet18,MobileNetV2
from skeleton2.projects import LogicModel, get_logger
from skeleton2.projects.others2 import NBAC, AUC
...
class Model2(LogicModel):
    def __init__(self, metadata):
        # set_random_seed_all(0xC0FFEE)
        super(Model2, self).__init__(metadata)
    ...
    def init_opt(self):
        ...
        scheduler_lr = skeleton2.optim.get_change_scale(
            skeleton2.optim.gradual_warm_up(
                skeleton2.optim.get_reduce_on_plateau_scheduler(
                    init_lr * lr_multiplier / warmup_multiplier,
                    patience=5, factor=.5, metric_name='train_loss'
                ),
                warm_up_epoch=5,
        ...
        self.optimizer_fc = skeleton2.optim.ScheduledOptimizer(
            params_fc,
            torch.optim.SGD,
            # skeleton.optim.SGDW,
        ...
        self.optimizer = skeleton2.optim.ScheduledOptimizer(
            params,
            torch.optim.SGD,
            # skeleton.optim.SGDW,
            steps_per_epoch=steps_per_epoch,
    ...
    def adapt(self, remaining_time_budget=None):
        ...
        #         self.dataloaders['valid'] is not None:
            ...
            policy = skeleton2.data.augmentations.autoaug_policy()
            ...
            for policy_search in range(num_policy_search):
                ...
                self.dataloaders['valid'].dataset.transform.transforms = original_valid_policy + [
                    ...
                    skeleton2.data.augmentations.Augmentation(
                        selected_policy
                    ),
            ...
            if len(searched_policy) > 0:
                ...
                policy = skeleton2.data.augmentations.remove_duplicates(policy)
    ...
    def epoch_train(self, epoch, train, model=None, optimizer=None):
        ...
        for step, (examples, labels) in enumerate(train):
            ...
            skeleton2.nn.MoveToHook.to((examples, labels), self.device, self.is_half)
    ...
    def prediction(self, dataloader, model=None, test_time_augmentation=True, detach=True, num_step=None):
        ...
        with torch.no_grad():
            ...
            # if self.is_fuse:
        #     print(self.results)
        #     results = np.sort(np.array([v['valid_score'] for v in self.results]))
        #     print(results)
        #     if 0 < self.info['loop']['test'] <= 10:
        #         results = self.results[-2:]
        #         predictions = np.array(
        #             [v['pred'] for v in results])  # self.info['loop']['epoch'] self.info['loop']['test']
        ...
        #         predictions = np.array(
        #             [v['pred'] for v in self.results])  # self.info['loop']['epoch'] self.info['loop']['test']
        #         res = np.mean(predictions, axis=0)
        #     else:
        #         res = predictions
        ...
        # return res
            ...
            if len(self.results) > 4:
                index = np.argsort(np.array([v['valid']['score'] for v in self.checkpoints]))
                index = sorted(index[::-1][:self.hyper_params['checkpoints']['keep']])
                list = [self.checkpoints[i]['epoch'] for i in index]
